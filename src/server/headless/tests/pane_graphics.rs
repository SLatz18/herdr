...
#[tokio::test]
async fn pixel_mouse_activation_follows_child_1016_not_direct_transport() {
...
#[tokio::test]
async fn pixel_mouse_activation_stays_off_without_child_1016() {
    let (mut server, _client_rx, _pane_id) = retained_test_server(b"\x1b[?1003h\x1b[?1006h");
    let (writer, control_rx, _render_rx) = test_client_writer();
    let client = server.clients.get_mut(&1).unwrap();
    client.writer = Some(writer);
    client.direct_graphics = false;
